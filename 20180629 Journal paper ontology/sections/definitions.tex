...
\subsubsection{A scenario consists of one or several events \autocite{vannotten2003updated, go2004blind, geyer2014, ulbrich2015, kahn1962, englund2016grand, schoemaker1993multiple, cuppens2002alert, bach2016modelbased}}
...
Although the description of the static environment is not a general prerequisite of a scenario, this is often included when speaking about traffic scenarios \autocite{geyer2014, ulbrich2015, elrofai2016scenario, ebner2011identifying, schuldt2013effiziente, althoff2017CommonRoad}.
...
%\Cref{fig:scenario} shows a schematic overview of the various components of a scenario. As shown in the second row of \cref{fig:scenario}, a scenario contains an ego vehicle, a dynamic environment, and a static environment. As an example, possible activities of the vehicle in front, which is part of the dynamic environment, are detailed by considering the \cstart state variables \cend `heading' and `speed'. Note that a definition of the term activity is given in \cref{sec:activity}. The two rows of the activities show more generic descriptions and more detailed descriptions, respectively, of some possible activities.
...
\begin{itemize}
	...
	\item In the field of hybrid systems theory, ``the continuous and discrete dynamics interact at `event' or `trigger' times when the continuous state \cstart [vector] \cend hits certain prescribed sets in the continuous state space'' \autocite{branicky1998hybridcontrol}. ``A hybrid system can be in one of several modes, [...], and the system switches from one mode to another due to the occurrence of events'' \autocite{deschutter2000optimal}.
...
\cstart We considered the definitions of event in hybrid systems and control and event-based control. Therefore, an event happens at some time instant rather than a time span. Note, however, that the timing itself may be undefined.\cend
...
\begin{definition}[Event] \label{def:event}
	An event marks the time instant at which the system reaches a specified threshold or at which a mode transition occurs, such that before and after an event, the state \cstart vector \cend of the system corresponds to two different modes.
\end{definition}
...
\cstart
Definition~2 indicates that the moment of an event can be defined in two different ways: (1) by the system reaching a threshold or (2) by a mode transition. \cend 
Both these types of event are related to hybrid systems theory \autocite{deschutter2003hybrid, heemels2012eventcontrol}.
The first type of event, i.e., related to the system reaching a threshold, is especially useful when describing test cases. 
For example, consider the ego vehicle approaching a pedestrian that is about to cross the road \autocite{seiniger2015test}. 
...
\cstart The second type of event, i.e., a mode transition, can, e.g., occur at the moment of a driver input.\cend 
...
\begin{itemize}
	...
	\item In a glossary for a scenario catalog development \autocite{catapult2018musicc}, an activity is defined as ``the state \cstart [vector] \cend of an object over an interval of time. An activity starts with an event and ends with another event.''
...
\subsubsection{An activity quantitatively describes the time evolution of a state \protect\cstart variable\protect\cend}
...
Therefore, an activity describes the time evolution of \cstart one or more state variables \cend, i.e., the trajectory of \cstart one or more state variables \cend over an inter-event time interval that corresponds to the activity, where the term state \cstart variable \cend is defined in \cref{sec:state variable}.

% Performed by something (an actor?)
\subsubsection{An activity is performed by an actor}
An activity quantitatively describes the time evolution of \cstart one or more state variables \cend and a state \cstart variable \cend corresponds to an actor, e.g., the acceleration of a vehicle, and, therefore, an activity is performed by an actor. Note that the actor might be the ego vehicle. 
...
\begin{definition}[Activity]
	\label{def:activity}
	An activity quantitatively describes the time evolution of \cstart one or more state variables \cend of an actor between two events.
\end{definition}
...
%Therefore, a scenario class refers to multiple scenarios with a common characteristic \autocite{elrofai2018scenario}.
...
We describe the formal relation between a scenario and a scenario category with the verb ``to \cstart comprise\cend'', denoted by \cstart$\comprises$\cend. If a specific scenario category $\scenariocategory$ is an abstraction of a specific scenario $\scenario$, then we say that the specific \cstart scenario category $\scenariocategory$ comprises that specific $\scenario$, or simply $\scenariocategory \comprises \scenario$\cend.  This is illustrated in \cref{fig:venn diagram scenario category}, where $\scenarioa$, $\scenariob$, and $\scenarioc$ represent scenarios and $\scenariocategorya$, $\scenariocategoryb$, and $\scenariocategoryc$ represent scenario categories. \cstart A given scenario category can comprise multiple scenarios, e.g., $\scenariocategoryb \comprises \scenarioa$ and $\scenariocategoryb \comprises \scenariob$ in \cref{fig:venn diagram scenario category}. 
% Explain scenario can fall into multiple scenario categories
Furthermore, multiple scenario categories can comprise a specific scenario. For example, in \cref{fig:venn diagram scenario category}, we have $\scenariocategorya \comprises \scenariob$, $\scenariocategoryb \comprises \scenariob$, and $\scenariocategoryc \comprises \scenariob$. \cend
...
The verb ``to \cstart include\cend'' is used to describe the relation between two scenario categories. A scenario category $\scenariocategoryb$ is said to \cstart include \cend a scenario category $\scenariocategorya$ \cstart if $\scenariocategoryb$ comprises all scenarios that are comprised in $\scenariocategorya$. In that case, we can write $\scenariocategoryb \includes \scenariocategorya$. Thus we have
...
When providing extra information on a piece of data, often tags are used \autocite{smith2007tagging}. A tag is a keyword or a term that helps describing an item. For example, items in a database can contain some tags that enable users to quickly obtain several items that share a certain characteristic described by a tag \autocite{craft2004tagging, vasquez2019controlling}. 
...
\begin{itemize}
	\item  The tags of a scenario can be helpful in determining which scenario categories \cstart do and do not comprise \cend the scenario.
