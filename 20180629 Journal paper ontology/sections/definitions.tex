...
% "Requirements"
Before providing the definition of the notion of scenario, the characteristics of the notion of scenario are listed as follows:
...
\begin{definition}[Activity]
	\label{def:activity}
	An activity quantitatively describes the state of an actor between two events.
\end{definition}
\cbend
...
We describe the formal relation between a scenario and a scenario class with the verb ``to fall into'', denoted by $\in$. If a specific scenario class $\mathcal{C}$ is an abstraction of a specific scenario $S$, then we say that the specific scenario $S$ falls into that specific scenario class $\mathcal{C}$, or simply $S \in \mathcal{C}$. Multiple scenarios can fall into a given scenario class. For example, consider the scenario class with the name ``Daytime'', see \cref{fig:venn diagram scenario class}. All scenarios that occur during daytime fall into the scenario class ``Daytime''.

\setlength{\venncircle}{7em}
\begin{figure}
	\centering
	\input{figures/venn_diagrams.tikz}
	\caption{The red and green circles correspond to the scenario classes ``Daytime'' and ``Rain'', respectively. Scenarios that occur during daytime with rain fall into both scenario classes ``Daytime'' and ``Rain''. The new scenario class ``Daytime and rain'' can be defined as the scenario class in which all scenarios that occur during daytime with rain fall. The scenario class ``Daytime and rain'' falls into the scenario classes ``Daytime'' and ``Rain''.}
...
For example, if a scenario contains the tag ``Daytime'', it is automatically recognized as a scenario that falls into the scenario class ``Daytime'' which is characterized by the single tag ``Daytime''. \cbend
...
\begin{itemize}
	...
	\item As opposed to proposed categorization of scenarios \cite{opdencamp2014cats, USDoT2007precrashscenarios, lenard2011typical, lara2019harmonized}, scenario classes do not need to be mutually exclusive.
\end{itemize}
