...
The first type of event, i.e., at the moment at which the system reaches a boundary, is especially useful when describing test cases. For example, consider a vehicle approaching a pedestrian (dummy) that is about to cross the road \cite{seiniger2015test}. Here, the event is at the moment that the distance between the vehicle and pedestrian is less than $x$ meters. At this event, the pedestrian (dummy) starts to cross the road such that the vehicle impacts the pedestrian (dummy) if it does not change its speed or direction \cite{seiniger2015test}.
