...
\cbstartb
According to \cref{def:scenario}, a scenario describes, among others, the static environment. When it comes to applying this definition in an ontology, it is possible that ``describing'' the static environment is done by simply ``referring'' to a static environment. This also holds for the other parts of a scenario. The advantage of ``referring'' is that these parts of the scenario can be reused for other scenarios. As an example, an OpenSCENARIO file, which has as primary use-case to describe the dynamic content of a scenario \cite{openscenario}, allows to refer to an OpenDRIVE file which describes a road network \cite{dupuis2010opendrive}. As we will see in \cref{sec:ontology}, in our proposed ontology, a scenario ``refers'' to a static environment, activities, actors, and events. 
\cbend
