...
\begin{itemize}
	...
	\cbstart
	\item In event-based control, a control action is computed when an event is triggered, as opposed to the more traditional approach where a control action is periodically computed \cite{heemels2012eventcontrol}. Here, the trigger is the moment at which the system (is about to) reach a certain boundary.
	\cbend
\end{itemize}
...
% Mention that there are basically two definitions. First definition more suitable for observations and inputs. Second definition more suitable for triggers for test cases.
According to \cref{def:event}, an event is either a moment at which the system reaches a boundary or a moment at which a mode transition occurs. This first type of event, i.e., the moment at which the system reaches a boundary, is related to events in event-based control \cite{heemels2012eventcontrol}, whereas the second type of event, i.e., the moment at which a mode transition occurs, is related to events in hybrid theory \cite{deschutter2000optimal}. Especially when designing scenarios for testing purposes, an event of the first type might trigger an event of the second type. For example, consider a vehicle approaching a zebra crossing with a pedestrian (dummy) that is about to cross the road at the zebra crossing. When the distance between the vehicle and the zebra crossing becomes less than 20 meters (event of the first type), the pedestrian (dummy) starts crossing the road at the zebra crossing (event of the second type).
