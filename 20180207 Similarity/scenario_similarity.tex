...
To illustrate the dissimilarity measures, three different velocity profiles are considered, see \cref{fig:example time series}. All three velocity profiles describe the velocity of a vehicle while braking. The average deceleration is $\unit[3.19]{m/s^2}$, $\unit[1.72]{m/s^2}$, and $\unit[1.63]{m/s^2}$, respectively. We will refer to the the profiles with $\textbf{v}_1$, $\textbf{v}_2$, and $\textbf{v}_3$, respectively. 
...
\begin{figure}
	...
	\caption{Three examples of time series which will be used to illustrate the dissimilarity measures. The velocity is sampled with $\unit[10]{Hz}$.}
...
A straightforward way to estimate the dissimilarity between two time series is to use an $L_p$ norm. This, however, does only work if the two time series have equal length. In case that the time series $\textbf{x}$ and $\textbf{y}$ do not have the same length, one of the time series can be re-sampled. This works well for a number of data sources \cite{keogh2003survey}. Let the time series $\bar{\textbf{y}}$ denote the re-sampled time series $\textbf{y}$, such that $\bar{\textbf{y}}$ contains $N$ samples, e.g., using linear interpolation. As a consequence, the elements $\bar{y}_i$ of $\bar{\textbf{y}}$ are not sampled with the same frequency as the time series $\textbf{x}$ and $\textbf{y}$. The $L_p$ norm is as follows:
\begin{equation} \label{eq:lp norm}
	d_{L_p}(\textbf{x}, \bar{\textbf{y}}) = \left( \sum_{i=1}^N |x_i - \bar{y}_i|^p \right)^{\frac{1}{p}},
\end{equation}
where $p \geq 0$. Not that with $p=2$, we obtain the Euclidean distance:
\begin{equation} \label{eq:euclidian}
	d_{\textup{Euclidian}}(\textbf{x}, \bar{\textbf{y}}) = \sqrt{\sum_{i=1}^N |x_i - \bar{y}_i|^p}.
\end{equation}
...
For our purposes, the Euclidean distances is not suitable. Because, in general, we have $N \ne M$, we need to re-sample one time series. Because of this, a strong braking action might look `similar' to very smooth braking, see \cref{fig:resampled profiles}. In \cref{fig:resampled profiles}, $\textbf{v}_1$ and $\textbf{v}_3$ are re-sample, such that they have the same amount of samples as $\textbf{v}_2$, resulting in $\bar{\textbf{v}}_1$ and $\bar{\textbf{v}}_3$, respectively. This results in $d_{\textup{Euclidian}}(\textbf{v}_2, \bar{\textbf{v}}_1)=\unit[13.3]{m/s}$ and $d_{\textup{Euclidian}}(\textbf{v}_2, \bar{\textbf{v}}_3)=\unit[14.7]{m/s}$. So even though $\textbf{v}_3$ looks more similar to $\textbf{v}_2$ than $\textbf{v}_1$, the Euclidean distance \cref{eq:euclidian} gives a higher score for $d_{\textup{Euclidian}}(\textbf{v}_2, \bar{\textbf{v}}_3)$ compared to $d_{\textup{Euclidian}}(\textbf{v}_2, \bar{\textbf{v}}_1)$.

\begin{figure}
	\centering
	\setlength\figureheight{0.4\linewidth}
	\setlength\figurewidth{0.48\linewidth}
	\subfloat[$\textbf{v}_2$ (blue) and $\bar{\textbf{v}}_1$ (orange).]{\input{figures/velocity_profiles_resampled21.tikz}}
	\hspace{0.04\linewidth}
	\subfloat[$\textbf{v}_2$ (blue) and $\bar{\textbf{v}}_3$ (orange).]{\input{figures/velocity_profiles_resampled23.tikz}}
...
\begin{equation}\label{eq:eucl features}
	d_{\textup{F}}(\textbf{x}, \textbf{y}) = \sqrt{ \sum_{k=1}^{\theta} \left( \hat{x}_k - \hat{y}_k \right)^2 }.
\end{equation}
