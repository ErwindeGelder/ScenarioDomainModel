...
\newcommand{\toauthornew}{\paragraph*{New comment:} \itshape}
\newcommand{\fromauthornew}{\paragraph*{New reply:} \normalfont}
...
\color{blue} Question: Should I also show the old comments? \color{black}
...
\fromauthornew The mentioned ontologies are used to describe specific tests, such that these tests can be executed in a virtual environment. In order to be used for testing in a virtual environment, these implementation describe scenarios on a quantitative level. We have updated the text to clarify this:
\cstart From the implementation side, there are several file formats and methods for scenario ontologies, e.g., OpenSCENARIO \autocite{openscenario} and CommonRoad \autocite{althoff2017CommonRoad}. 
These implementations are used to describe specific tests for automated vehicles that can be executed in a virtual environment and, therefore, these implementations describe scenarios at a quantitative level.
...
\fromauthornew \color{blue}I do not understand this comment. Why do we need to mention what kind of hybrid system our definition is based on? The comment mentions ``their model'', but I do not see what the reviewer refers to.\color{black}
...
\fromauthornew The definition of the term \emph{event} is given in Definition~2. The added text is used to clarify the definitions by illustrating it through two short examples.
...
\fromauthornew We have removed the figure with the overlapping circles.
...
\fromauthornew The reviewer correctly points out that we use terms that are also used in system theory, such as \emph{event} and \emph{state}. These terms are also commonly used in the field of safety assessment. The terms that we use\footnote{In the nomenclature, we mention the terms \emph{ego vehicle}, \emph{actor}, \emph{state}, \emph{model}, \emph{mode}, \emph{act}, \emph{static environment}, and \emph{dynamic environment}. Furthermore, we define the terms \emph{scenario}, \emph{event}, \emph{activity}, and \emph{scenario category}.} except the term \emph{scenario category}\footnote{Note that the term \emph{scenario category} will be used in a future release of OpenSCENARIO.}, are commonly used in the field of safety assessment \autocite{catapult2018musicc,catapult2018regulating,sigsim2019glossary,openscenario,ulbrich2015,geyer2014}.  
...
\fromauthornew We have made Fig.\ 3 smaller.
