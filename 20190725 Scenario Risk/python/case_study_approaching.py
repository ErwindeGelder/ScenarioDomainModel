...
from simulation import SimulationApproaching, HDM, ACC, ACCHDM, hdm_approaching_pars, \
...
def parameters_approaching() -> np.ndarray:
    ...
    """
    data = DataBaseEmulator(os.path.join("data", "5_scenarios", "approaching_vehicle.json"))
    pars = []
    for i in range(len(data.collections["scenario"])):
        ...
        vtarget = scenario.get_state(scenario.get_actor_by_name("target vehicle"),
                                     StateVariable.LON_TARGET, scenario.time["start"])[0]
        if vego > vtarget:
...
def check_validity_approaching(par: Union[List, np.ndarray]) -> bool:
    ...
    # Target speed must not be negative and less than ego speed.
    if par[1] < 0 or par[1] >= par[0]:
        return False
