...
Let $\sampleaccevent$ and $\sampledecevent$ denote the sample step of the most recent accelerating and decelerating event, respectively. A cruising event happens at sample step $\sample$ if the vehicle is not performing a cruising activity and if any of the following conditions is true:
\begin{itemize}
...
\cstarta
\Cref{fig:ego lane change} illustrates a hypothetical lane change of the ego vehicle. It shows that the distance towards the left line changes when the ego vehicle crosses the line, such that the conditions \cref{eq:potential right a,eq:potential right b} are satisfied. In \cref{fig:ego lane change}, $\samplestartright{\sample}$ and $\sampleendright{\sample}$ are denoted by the black vertical lines.
...
To determine the start of the lane change, the lateral speed should be below a certain threshold (controlled by $\lanechangespeed$) or the lateral movement should be above a certain threshold (controlled by $\factorgoalmax$). 
