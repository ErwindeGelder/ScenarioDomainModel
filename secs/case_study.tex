...
In \cref{sec:ngsim metric}, we demonstrate how our the \ac{ourmethod} method can be used to create \iac{ssm} that calculates the risk of a collision in a longitudinal interaction between two vehicles.
...
\textcite{wang2014evaluation} provide \iac{ssm} that calculates the probability of a collision under certain assumptions. 
...
\Cref{fig:ws comparison} shows the results of the comparison between the measure of \textcite{wang2014evaluation} and the measure derived using our proposed approach in \cref{sec:method}.
...
To illustrate the \ac{ourmethod} method, we apply it to derive \iac{ssm} that calculates the risk of a collision in a longitudinal interaction between two vehicles.
...
In this section, we demonstrate an approach for benchmarking \iac{ssm} that is based on the so-called \emph{expected risk tendencies} of \textcite{mullakkal2017comparative}.
