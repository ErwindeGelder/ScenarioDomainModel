...
In \cref{sec:ngsim metric}, we demonstrate how our proposed method can be used to create \iac{ssm} that calculates the risk of a collision in a longitudinal interaction between two vehicles.
...
To illustrate the proposed method, we applied it to derive \iac{ssm} that calculates the risk of a collision in a longitudinal interaction between two vehicles.
...
\subsection{Benchmarking an \acs{ssm} with expected causal tendencies}
\label{sec:tendencies}

In this section, we demonstrate an approach for benchmarking \iac{ssm} that is based on the so-called \emph{expected causal tendencies} of \textcite{mullakkal2017comparative}.
