...
% What is lacking?
\cstarta Typically, \acp{ssm} rely on assumptions regarding the future evolution of traffic participant trajectories to generate a measure of risk. \cenda
...
The \ac{ourmethod} method is used to derive \acp{ssm} that \cenda\cstartc can be used to calculate in real-time the \cendc\cstarta probability of a specific event (e.g., collision). 
Because we adopt a data-driven approach to predict the possible future evolutions of traffic participant trajectories, less assumptions are needed.
...
\cstarta Instead, we provide a method for benchmarking our derived \ac{ssm} with respect to expected risk trends.
The application of the benchmarking illustrates that the \ac{ssm} matches the expected risk trends. \cenda
