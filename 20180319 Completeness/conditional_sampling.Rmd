...
weights <- exp(-0.5 * ((pars[, 1] - amean)^2 + 
                       (pars[, 2] - vend)^2) / h^2)
...
samples <- (pars[integers, 3] + h*rnorm(n))

# Show the result
xpdf <- seq(xmin, xmax, length.out = npdf)
ypdf <- numeric(npdf)
for (i in 1:npdf) {
    ypdf[i] <- sum(exp(-.5 * ((pars[, 1] - amean)^2 + 
                              (pars[, 2] - vend)^2 +
                              (pars[, 3] - xpdf[i])^2) / h^2))
}
...
weights <- numeric(dim(pars)[1])
for (i in 1:dim(pars)[1]) {
    weights[i] <- exp(-.5 * sum((xfixed - xbar[i,])^2) / h^2 )
}
...
xvariable <- xtilde[integers] + h*rnorm(n)
