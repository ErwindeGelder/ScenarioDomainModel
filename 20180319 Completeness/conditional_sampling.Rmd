...
pars[,1] <- pars[,3] / pars[,1]  # amean, vdiff, vend
```
...
hist(samples, breaks = 100, freq = FALSE, xlim = c(xmin, xmax), 
     xlab = "Speed difference [m/s]")
...
## Conditional sampling with amean=1, vstart=20 and simple bandwidth
...
ypdf <- ypdf / (sum(ypdf) * mean(diff(xpdf)))
hist(samples[,3], breaks = 100, freq = FALSE, xlim = c(xmin, xmax), 
     xlab = "Speed difference [m/s]")
lines(xpdf, ypdf, col="red")
```
