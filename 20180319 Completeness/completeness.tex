...
\begin{figure}
	...
	\begin{minipage}{\linewidth}
		\centering
		\input{figures/artificial_data_acc.tikz}%
		\subcaption{Acceleration.}	
	\end{minipage}
...
The Kullback-Leibler divergence is always positive and zero if and only if $\hat{f}(x;n+m) = \hat{f}(x;n)$. Wang~\etal\ \cite{wang2017much} determine the proper amount of driving data such that $KL({\hat{f}(x;n+m)} || {\hat{f}(x;n)})$ changes very slightly if more data samples are to be added, i.e.,
...
\begin{remark}
	Although Wang~\etal\ use \cref{eq:completeness wang} to determine the appropriate number of samples $n$, it seems a bad choice as \cref{eq:completeness wang} is satisfied if the difference between $\hat{f}(x;n+m)$ and $\hat{f}(x;n)$ is almost similar to the difference between $\hat{f}(x;n+m)$ and $\hat{f}(x;n+2m)$. A better should would be to use
