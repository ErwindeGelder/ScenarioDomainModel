...
class PhysicalThing(QuantitativeThing):
    ...
    Attributes:
        ...
        category (PhysicalThingCategory): The qualitative counterpart.
        properties(dict): All properties of the physical thing.
    """
    def __init__(self, category: PhysicalThingCategory, properties: dict = None, **kwargs):
        if properties is None:
            properties = dict()
        check_for_type("category", category, PhysicalThingCategory)
        check_for_type("properties", properties, dict)

        QuantitativeThing.__init__(self, **kwargs)
        self.category = category  # type: PhysicalThingCategory
...
def _physical_thing_props_from_json(json: dict, attribute_objects: DMObjects,
                                    category: Union[PhysicalThingCategory, bool] = None) -> dict:
    ...
    if category is not False:
        props.update(_attributes_from_json(json, attribute_objects,
                                           dict(category=(_physical_thing_category_from_json,
...
def _physical_thing_from_json(json: dict, attribute_objects: DMObjects,
                              category: PhysicalThingCategory = None) -> PhysicalThing:
...
def physical_thing_from_json(json: dict, attribute_objects: DMObjects = None,
    ...
    PhysicalThingCategory needs to be fully defined instead of only the unique
    ID. Alternatively, the PhysicalThingCategory can be passed as optional
    argument. In that case, the PhysicalThingCategory does not need to be
